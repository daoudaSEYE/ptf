...
@include('includes/header/header')
	...
	<div class="site-breadcrumb">
		<div class="container">
			...
			<span><i class="fa fa-angle-right"></i>vente</span>
		</div>
	</div>
 <!--   <div class=" col-md-12">
				...
				<div class="col-lg-4 col-md-6">
					...
					<div class="feature-item">
						<div class="feature-pic set-bg" data-setbg="img/feature/1.jpg">
							@if($vente->categorie === "vente")
								<div class="sale-notic">{{$vente->categorie}}</div>
							@elseif($vente->categorie === "location")
								<div class="rent-notic">{{$vente->categorie}}</div>	
							@endif()
							<?php
							$now   = time();
							$dateEvent = strtotime($vente->created_at);
								// Test de la fonction
						...
						<div class="feature-text">
							<div class="text-center feature-title">
								<h5>{{$vente->nom}}</h5>
								<p><i class="fa fa-map-marker"></i> {{$vente->addresse}}</p>
											<?php if($vente->categorie==="location"):?>
												<?php
													$now   = strtotime(date('y-m-d'));
													$explode=explode('-',$vente->created_at);
													$explodeDay=explode(' ',$explode[2]);
													$created_at=$explode[0]."-".$explode[1]."-".$explodeDay[0];
													$offer_end=date('y-m-d', strtotime($created_at."+ ".$vente->temps_location." day"));
							...
							<div class="row">
								<a href="/{{$vente->ref}}" class="col-md-8 col-xs-8 room-price">{{$vente->prix}} FCFA
									@if($vente->categorie === "location")
										/ Mois
									@endif()
								</a>
								<a href="/{{$vente->ref}}" class="col-md-4 col-xs-4  consulter-feature">consulter</a>
							</div>
							</div>
					</div>
				</div>
