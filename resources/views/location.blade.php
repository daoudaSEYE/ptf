...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <title>P.T.F Immo | Location</title>
<!-- meta tags -->
<meta charset="utf-8">
...
@include('includes/header/header')
	...
	<div class="site-breadcrumb">
		<div class="container">
			...
			<span><i class="fa fa-angle-right"></i>Location</span>
		</div>
	</div>
 <!--   <div class=" col-md-12">
				...
				<div class="col-lg-4 col-md-6">
					...
					<div class="feature-item">
						<div class="feature-pic set-bg" data-setbg="img/feature/1.jpg">
							@if($location->categorie === "vente")
								<div class="sale-notic">{{$location->categorie}}</div>
							@elseif($location->categorie === "location")
								<div class="rent-notic">{{$location->categorie}}</div>	
							@endif()
							<?php
							$now   = time();
							$dateEvent = strtotime($location->created_at);
								// Test de la fonction
						...
						<div class="feature-text">
							<div class="text-center feature-title">
								<h5>{{$location->nom}}</h5>
								<p><i class="fa fa-map-marker"></i> {{$location->addresse}}</p>
											<?php if($location->categorie==="location"):?>
												<?php
													$now   = strtotime(date('y-m-d'));
													$explode=explode('-',$location->created_at);
													$explodeDay=explode(' ',$explode[2]);
													$created_at=$explode[0]."-".$explode[1]."-".$explodeDay[0];
													$offer_end=date('y-m-d', strtotime($created_at."+ ".$location->temps_location." day"));
							...
							<div class="row">
								<a href="/{{$location->ref}}" class="col-md-8 col-xs-8 room-price">{{$location->prix}} FCFA
									@if($location->categorie === "location")
										/ Mois
									@endif()
								</a>
								<a href="/{{$location->ref}}" class="col-md-4 col-xs-4  consulter-feature">consulter</a>
							</div>
							</div>
					</div>
				</div>
